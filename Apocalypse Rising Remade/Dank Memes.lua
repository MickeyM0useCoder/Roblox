local Events = game.Lighting.Remote
local RblxEnv = getrenv()
local Serial = debug.getupvalues(getrawmetatable(RblxEnv._G).__index)[2]

local untrek = Serial.untrek
local Key = untrek(Serial.gunsound.SoundId)

function fireserver(A, ...)
    Events[tostring(A)]:FireServer(Key,...)
end
...
Events.AddDamageSelf:FireServer(game.Players.LocalPlayer.Character.Humanoid, math.huge)
...
Events.Damage:FireServer(Key, game.Players.LocalPlayer.Character.Humanoid, math.huge)
end)
...
repeat
Events.PlaceC4:FireServer(material, pos, true)
wait()
until C4On == false
...
plr = game.Players.LocalPlayer.playerstats
    fireserver("ChangeValue", plr.Hunger, math.huge)
    fireserver("ChangeValue", plr.Thirst, math.huge)
end)
...
local player = GetPlayer(PlayerSelect.Text)
    fireserver("ChangeParent", player.Character, nil)
end)
...
local playername = GetPlayer(PlayerSelect.Text)
...
o = GetPlayer(PlayerSelect.Text)
game.Lighting.Remote:WaitForChild("TeleportRequest"):InvokeServer(Key,"Me To", math.floor(tick() % 1 * 100000), o)
end)
...
for _, players in next, game.Players:GetPlayers() do
Events.SendMessage:FireServer(Key, players, 'Red', MessageBox.Text)
end
end)
-------
...
BlueButton.MouseButton1Down:connect(function()
    for _, players in next, game.Players:GetPlayers() do
        Events.SendMessage:FireServer(Key, players, 'Blue', MessageBox.Text)
        end
end)
-------
...
GreenButton.MouseButton1Down:connect(function()
    for _, players in next, game.Players:GetPlayers() do
        Events.SendMessage:FireServer(Key, players, 'Green', MessageBox.Text)
        end
end)
-------
...
YellowButton.MouseButton1Down:connect(function()
    for _, players in next, game.Players:GetPlayers() do
        Events.SendMessage:FireServer(Key, players, 'Yellow', MessageBox.Text)
        end
end)
-------
...
WhiteButton.MouseButton1Down:connect(function()
    for _, players in next, game.Players:GetPlayers() do
        Events.SendMessage:FireServer(Key, players, 'White', MessageBox.Text)
        end
end)
-------
...
while _G.BoolE do
    for _, players in next, game.Players:GetPlayers() do 
        Events.SendMessage:FireServer(Key, players, Colors[math.random(1, #Colors)], MessageBox.Text) 
        wait()
    end
end
end)
...
while tspamBool do
wait()
Events.Chat:FireServer("Global", words[math.random(#words)])
end
    else
        tspamBool = false
    end
